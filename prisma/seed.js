...
async function main() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (attempts >= 10)
            break; // No more unreferred customers
        ...
        await prisma.pointEntry.create({
            data: {
                userId: referrer.id,
                delta: 5000,
                source: prisma_1.PointsSource.REFERRAL_REWARD,
    ...
    const eventCategories = [prisma_1.EventCategory.TECH, prisma_1.EventCategory.MUSIC, prisma_1.EventCategory.BUSINESS, prisma_1.EventCategory.EDUCATION, prisma_1.EventCategory.SPORTS, prisma_1.EventCategory.ART, prisma_1.EventCategory.COMMUNITY, prisma_1.EventCategory.OTHER];
    const eventStatuses = [prisma_1.EventStatus.PUBLISHED, prisma_1.EventStatus.DRAFT, prisma_1.EventStatus.CANCELED];
    const events = [];
    for (let i = 1; i <= 15; i++) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const voucher = await prisma.voucher.create({
            data: {
                ...
                eventId: event.id,
                organizerId: organizer.id,
                discountType: getRandomItem([prisma_1.DiscountType.PERCENT, prisma_1.DiscountType.AMOUNT]),
                discountValue: getRandomItem([prisma_1.DiscountType.PERCENT, prisma_1.DiscountType.AMOUNT]) === prisma_1.DiscountType.PERCENT ? getRandomNumber(10, 50) : getRandomNumber(25000, 100000),
    ...
    for (let i = 0; i < 15; i++) {
        ...
        const coupon = await prisma.coupon.create({
            data: {
                ...
                userId: customer.id,
                discountType: getRandomItem([prisma_1.DiscountType.PERCENT, prisma_1.DiscountType.AMOUNT]),
                discountValue: getRandomItem([prisma_1.DiscountType.PERCENT, prisma_1.DiscountType.AMOUNT]) === prisma_1.DiscountType.PERCENT ? getRandomNumber(5, 25) : getRandomNumber(10000, 50000),
    ...
    const transactionStatuses = [prisma_1.TransactionStatus.WAITING_PAYMENT, prisma_1.TransactionStatus.WAITING_ADMIN_CONFIRMATION, prisma_1.TransactionStatus.DONE, prisma_1.TransactionStatus.REJECTED, prisma_1.TransactionStatus.EXPIRED, prisma_1.TransactionStatus.CANCELED];
    const transactions = [];
    ...
    for (let i = 1; i <= 30; i++) {
        ...
        let pointsUsed = 0;
        if (voucher) {
            if (voucher.discountType === prisma_1.DiscountType.PERCENT) {
        ...
        if (coupon) {
            if (coupon.discountType === prisma_1.DiscountType.PERCENT) {
        ...
        if (status === prisma_1.TransactionStatus.WAITING_ADMIN_CONFIRMATION && Math.random() > 0.3) {
        ...
        if (pointsUsed > 0) {
            await prisma.pointEntry.create({
                data: {
                    ...
                    transactionId: transaction.id,
                    delta: -pointsUsed,
                    source: prisma_1.PointsSource.PURCHASE_REDEEM
                }
            });
        ...
        if (status === prisma_1.TransactionStatus.DONE) {
        ...
        if (status === prisma_1.TransactionStatus.DONE || status === prisma_1.TransactionStatus.WAITING_ADMIN_CONFIRMATION) {
        ...
        if (voucher && (status === prisma_1.TransactionStatus.DONE || status === prisma_1.TransactionStatus.WAITING_ADMIN_CONFIRMATION)) {
        ...
        // Update coupon usage
        if (coupon && (status === prisma_1.TransactionStatus.DONE || status === prisma_1.TransactionStatus.WAITING_ADMIN_CONFIRMATION)) {
    ...
    for (let i = 0; i < 15; i++) {
        ...
        const notification = await prisma.emailNotification.create({
            data: {
                ...
                type: getRandomItem([prisma_1.NotificationType.TRANSACTION_ACCEPTED, prisma_1.NotificationType.TRANSACTION_REJECTED]),
                subject: `Transaction Update for ${event.title}`,
                body: `This is a notification about your transaction for ${event.title}`,
                status: getRandomItem([prisma_1.NotificationStatus.PENDING, prisma_1.NotificationStatus.SENT, prisma_1.NotificationStatus.FAILED])
            }
        });
    ...
    for (let i = 0; i < 25; i++) {
        const customer = getRandomItem(customers);
        const sources = [prisma_1.PointsSource.REFERRAL_REWARD, prisma_1.PointsSource.PURCHASE_REDEEM, prisma_1.PointsSource.ROLLBACK, prisma_1.PointsSource.ADMIN_ADJUSTMENT];
        const source = getRandomItem(sources);
        let delta = 0;
        switch (source) {
            case prisma_1.PointsSource.REFERRAL_REWARD:
                delta = 5000;
                break;
            case prisma_1.PointsSource.PURCHASE_REDEEM:
                delta = -getRandomNumber(1000, 10000);
                break;
            case prisma_1.PointsSource.ROLLBACK:
                delta = getRandomNumber(1000, 5000);
                break;
            case prisma_1.PointsSource.ADMIN_ADJUSTMENT:
        ...
        await prisma.pointEntry.create({
            data: {
                ...
                delta: delta,
                source: source,
                expiresAt: source === prisma_1.PointsSource.PURCHASE_REDEEM ? null : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000)
            }
        });
        // Update customer points balance
        await prisma.user.update({

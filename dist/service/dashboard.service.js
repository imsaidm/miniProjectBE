...
class DashboardService {
     ...
     */
    async getEventCount(organizerId) {
        try {
            return await prisma_1.prisma.event.count({
                where: { organizerId }
            });
        }
        catch (error) {
     ...
     */
    async getTicketSalesCount(organizerId) {
        try {
            return await prisma_1.prisma.transaction.count({
                where: {
                    event: { organizerId },
                    status: 'DONE'
                }
     ...
     */
    async getMonthlyRevenue(organizerId) {
        try {
     ...
     */
    async getYearlyRevenue(organizerId) {
        try {
            const yearlyRevenue = await prisma_1.prisma.$queryRaw `
        SELECT 
     ...
     */
    async getDailyRevenue(organizerId) {
        try {
            const currentYear = new Date().getFullYear();
            const currentMonth = new Date().getMonth() + 1;
            const dailyRevenue = await prisma_1.prisma.$queryRaw `
        SELECT 
     ...
     */
    async getDashboardReport(organizerId, period) {
        try {
            // Validate input
            if (!organizerId || organizerId <= 0) {
                throw new Error('Invalid organizer ID');
    ...
    async getEventAttendees(eventId) {
        try {
            ...
            return await prisma_1.prisma.attendance.findMany({
                where: { eventId },
                include: {
                    user: {
                        select: {
     ...
     */
    async getAnalyticsData(organizerId) {
        try {
            // Validate input
            if (!organizerId || organizerId <= 0) {
                throw new Error('Invalid organizer ID');
